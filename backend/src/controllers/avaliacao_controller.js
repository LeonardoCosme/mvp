...
import { Agendamento, Contratante, Avaliacao } from "../models/index.js";

/**
 * POST /api/avaliacoes
 * Body: { agendamentoId, nota(1..5), comentario? }
 */
export async function create(req, res) {
  try {
    ...
    if (!agId) {
      return res.status(400).json({ error: "agendamentoId obrigatório" });
    }
    if (!Number.isInteger(n) || n < 1 || n > 5) {
      return res
        .status(400)
        .json({ error: "nota deve ser um inteiro de 1 a 5" });
    }
    ...
    if (!contr) {
      return res.status(403).json({ error: "Apenas contratantes" });
    }

    // 🔹 agendamento precisa ser dele e estar concluído
    ...
    if (ag.status !== "concluida" || !ag.endAt) {
      return res
        .status(400)
        .json({ error: "Só é possível avaliar um agendamento concluído" });
    }
    if (!ag.prestadorId) {
      return res
        .status(400)
        .json({ error: "Agendamento sem prestador para avaliar" });
    }
...
/**
 * GET /api/avaliacoes/resumo/:prestadorId
 * Retorna média e distribuição de notas do prestador.
 */
export async function resumoPrestador(req, res) {
  try {
    const prestadorId = Number(req.params.prestadorId);
    if (!prestadorId) {
      return res.status(400).json({ error: "prestadorId inválido" });
    }
